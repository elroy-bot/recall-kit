...
def test_remember_command(cli_runner, temp_db):
    """Test the 'remember' command."""
    result = cli_runner.invoke(
        cli,
        [
            "--connection-string", temp_db,
            "remember",
...
def test_search_command(cli_runner, temp_db):
...
def test_search_json_output(cli_runner, temp_db):
...
def test_consolidate_command(cli_runner, temp_db):
...
def test_export_command(cli_runner, temp_db):
...
def test_stats_command(cli_runner, temp_db):
...
def test_chat_command(mock_input, cli_runner, temp_db):
    ...
    assert result.exit_code == 0, f"Exit code: {result.exit_code}, Output: {result.output.replace('\n', ' ')}"
    assert "Recall Kit Chat" in result.output
    assert "Using model: gpt-3.5-turbo" in result.output


@patch("recall_kit.cli.input")
def test_chat_help_command(mock_input, cli_runner, temp_db):
...
@patch("recall_kit.cli.input")
def test_chat_remember_command(mock_input, cli_runner, temp_db):
...
@patch("recall_kit.cli.input")
def test_chat_search_command(mock_input, cli_runner, temp_db):
...
@patch("recall_kit.cli.input")
def test_chat_clear_command(mock_input, cli_runner, temp_db):
...
def test_chat_completion(mock_chat_completion, mock_input, cli_runner, temp_db):
    ...
    assert len(kwargs["messages"]) == 3  # System message + user message + assistant message
