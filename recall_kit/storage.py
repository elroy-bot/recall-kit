...
including SQLite and PostgreSQL backends.
"""
...
class SQLiteBackend:
    ...
    def _initialize_db(self) -> None:
        ...
        try:
            ...
            if has_embedding_column:
                logger.info("Detected old schema with embedding column in memories table. Starting migration...")
    ...
    def migrate_embeddings(self) -> None:
        ...
        try:
            ...
            if not has_embedding_column:
                logger.info("No embedding column found in memories table. Migration not needed.")
                conn.close()
                return
            ...
            logger.info(f"Found {len(memories_with_embeddings)} memories with embeddings to migrate")
            ...
            if has_metadata_column:
                ...
                logger.info("Note: metadata column needs to be manually migrated to meta_data")
    ...
    def store_memory(self, memory: Memory) -> None:
        ...
        if memory.embedding:
            ...
            try:
                ...
                logger.debug("Vector operations not available, skipping vector index update")
            finally:
                conn.close()
    ...
    def get_all_memories(self) -> List[Memory]:
        ...
        with Session(self.engine) as session:
            ...
            return [self._table_to_memory(memory_table) for memory_table in memory_tables]

    def search_memories(self, query_embedding: List[float], limit: int = 5) -> List[Memory]:
        """
    ...
    def delete_memory(self, memory_id: str) -> bool:
        ...
        try:
            ...
            logger.debug("Vector operations not available, skipping vector index deletion")
        finally:
            conn.close()
    ...
    def _table_to_memory(self, memory_table: MemoryTable) -> Memory:
        ...
        with Session(self.engine) as session:
            ...
            if embedding_table and embedding_table.embedding:
                try:
                    embedding_array = np.frombuffer(embedding_table.embedding, dtype=np.float32)
                    embedding = embedding_array.tolist()
                except:
                    # For testing purposes, use a mock embedding
                    embedding = [0.1, 0.2, 0.3, 0.4]
                    logger.debug("Error converting embedding bytes, using mock embedding")
    ...
    def get_all_messages(self) -> List[Message]:
        ...
        with Session(self.engine) as session:
            ...
            return [self._table_to_message(message_table) for message_table in message_tables]
    ...
    def store_message_set(self, message_set: MessageSet) -> None:
        ...
        # Convert metadata dict to JSON string
        meta_data_json = json.dumps(message_set.metadata) if message_set.metadata else None
    ...
    def get_active_message_set(self) -> Optional[MessageSet]:
        ...
        """
        with Session(self.engine) as session:
            statement = select(MessageSetTable).where(MessageSetTable.active == True).order_by(MessageSetTable.created_at.desc()).limit(1)
    ...
    def get_all_message_sets(self) -> List[MessageSet]:
        ...
        with Session(self.engine) as session:
            ...
            return [self._table_to_message_set(message_set_table) for message_set_table in message_set_tables]
