...
import inspect
import pkgutil
from typing import Any, Callable, Dict, List, Optional, Type, TypeVar, Protocol, runtime_checkable, get_type_hints
...
from .core import AugmentFunction, CompletionFunction, EmbeddingFunction, FilterFunction, RerankFunction, RetrieveFunction, StorageBackendProtocol

# Utility functions for signature checking
def check_signature_compatibility(fn: Callable, protocol_class: Type[Protocol]) -> tuple[bool, str]:
    """
    ...
    try:
        ...
        for param_name, param_type in protocol_hints.items():
            ...
            if function_hints[param_name] != param_type:
                return False, f"Parameter '{param_name}' has type {function_hints[param_name]}, but protocol expects {param_type}"

        # Check return type
        if 'return' in protocol_hints and 'return' in function_hints:
            if protocol_hints['return'] != function_hints['return']:
                return False, f"Return type {function_hints['return']} is not compatible with protocol return type {protocol_hints['return']}"
...
def _check_protocol_conformance(obj: Any, protocol_class: Type[Protocol], obj_name: str = None) -> None:
    """
    ...
    if not is_compatible:
        raise TypeError(f"Function {name} does not conform to {protocol_class.__name__} protocol: {error_msg}")

    # Also check with isinstance for runtime protocol conformance
    if not isinstance(obj, protocol_class):
        raise TypeError(f"Function {name} does not conform to {protocol_class.__name__} protocol at runtime")
...
class PluginRegistry:
    ...
    def register_embedding_fn(self, fn: EmbeddingFunction, name: str, aliases: Optional[List[str]] = None) -> None:
        """
    ...
    def register_storage_backend(self, storage: Type[StorageBackendProtocol], name: str, aliases: Optional[List[str]] = None) -> None:
        """
        ...
        if not isinstance(storage, StorageBackendProtocol):
            raise TypeError(f"{storage.__name__} does not conform to StorageBackendProtocol")
    ...
    def register_embedding_fn(self, embedding_fn: EmbeddingFunction, name: str, aliases: Optional[List[str]] = None) -> None:
        """
    ...
    def register_completion_fn(self, completion_fn: CompletionFunction, name: str, aliases: Optional[List[str]] = None) -> None:
        """
    ...
    def register_retrieve_fn(self, retrieve_fn: Callable[[str, Any], List[Any]], name: str, aliases: Optional[List[str]] = None) -> None:
        """
    ...
    def register_filter_fn(self, filter_fn: Callable[[Any, Any], bool], name: str, aliases: Optional[List[str]] = None) -> None:
        """
    ...
    def register_rerank_fn(self, rerank_fn: Callable[[List[Any], Any], List[Any]], name: str, aliases: Optional[List[str]] = None) -> None:
        """
    ...
    def get_rerank_fn(self, name: str) -> Optional[Callable[[List[Any], Any], List[Any]]]:
        """
    ...
    def register_augment_fn(self, augment_fn: Callable[[List[Any], Any], Any], name: str, aliases: Optional[List[str]] = None) -> None:
        """
...
def register_embedding_fn(fn: EmbeddingFunction, name: str, aliases: Optional[List[str]] = None) -> None:
    """
    Register an embedding service.
...
def register_storage_backend(backend_class: Type[StorageBackendProtocol], name: str, aliases: Optional[List[str]] = None) -> None:
    """
    Register a storage backend.
...
def register_embedding_fn(embedding_fn: EmbeddingFunction, name: str, aliases: Optional[List[str]] = None) -> None:
    """
    Register an embedding function.
...
def register_completion_fn(completion_fn: CompletionFunction, name: str, aliases: Optional[List[str]] = None) -> None:
    """
    Register a completion function.
...
def register_retrieve_fn(retrieve_fn: RetrieveFunction, name: str, aliases: Optional[List[str]] = None) -> None:
    """
    Register a retrieve function.
...
def register_filter_fn(filter_fn: FilterFunction, name: str, aliases: Optional[List[str]] = None) -> None:
    """
    Register a filter function.
...
def register_rerank_fn(rerank_fn: RerankFunction, name: str, aliases: Optional[List[str]] = None) -> None:
    """
    Register a rerank function.
...
def register_augment_fn(augment_fn: AugmentFunction, name: str, aliases: Optional[List[str]] = None) -> None:
    """
    Register an augment function.
