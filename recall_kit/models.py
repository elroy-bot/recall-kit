...
class MemorySource(BaseModel):
    ...
    title: str = Field(..., description="A title or brief description of the memory source")
    address: str = Field(..., description="A unique identifier for the source (e.g., conversation:date:message:id)")
...
class Memory(BaseModel):
    ...
    embedding: Optional[List[float]] = Field(None, description="Vector embedding of the memory text")
    source_address: Optional[str] = Field(None, description="Address of the source that created this memory")
    parent_ids: List[str] = Field(default_factory=list, description="IDs of parent memories if this is a consolidated memory")
    created_at: datetime.datetime = Field(default_factory=datetime.datetime.now, description="When the memory was created")
    relevance: float = Field(default=0.0, description="Relevance score (set during retrieval)")
    active: bool = Field(default=True, description="Whether this memory is active (available for search)")
...
class Message(BaseModel):
    ...
    created_at: datetime.datetime = Field(default_factory=datetime.datetime.now, description="When the message was created")
...
class MessageSet(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique identifier for the message set")
    message_ids: List[str] = Field(default_factory=list, description="IDs of messages in this set")
    active: bool = Field(default=True, description="Whether this message set is active")
    created_at: datetime.datetime = Field(default_factory=datetime.datetime.now, description="When the message set was created")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata about the message set")
